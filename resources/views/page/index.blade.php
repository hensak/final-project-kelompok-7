...
@section('content')
  <div class="container-fluid">
    <form action="/thread/create" method="post">
      ...
      @enderror
      <div>
        <button type="submit" class="btn btn-primary" @guest disabled @endguest>Continue</button>
      </div>
    </form>
  </div>
<hr/>
      ...
      <div class="card-body">
        <h5 class="card-title h3">{{$item->title}}</h5>
          ...
          <div class="row d-flex justify-content-end w-100">
            <div class="col-md-4">
              <a href="/thread/{{$item->id}}" class="btn btn-primary">Details</a>
            </div>
            <div class="col-md-2">Category: {{$item->category->nama}}</div>
            <div class="col-md-4">Posted at: {{$item->date}}</div>
            <div class="col-md-2">
              <a @auth href="/thread/{{$item->id}}" @endauth>
